...
- GroupNorm usage is optional (default: disabled) and no longer silently falls back.
...
def make_norm(channels: int, norm_groups: int, eps: float = 1e-6) -> nn.Module:
    """
    Optional GroupNorm.
    - norm_groups <= 0  -> Identity
    - else: GroupNorm(norm_groups, channels) with strict divisibility.
    """
    if norm_groups <= 0:
        return nn.Identity()
...
class WindowAttention(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if use_sdpa:
            out = F.scaled_dot_product_attention(q, k, v, attn_mask=attn_mask)  # (BnW, heads, T, head_dim)
        else:
...
class SparseTopKMoE(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        # The aux loss is NOT applied automatically; training code may add:
        # total_loss = image_loss + aux_loss_weight * model.get_aux_loss()
        return y
